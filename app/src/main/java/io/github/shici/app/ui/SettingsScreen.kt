...
@Composable fun SettingsScreen(state: AppState, appearance: (Appearance) -> Unit, retention: (Double) -> Unit,
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState())) {
        ...
        Column(Modifier.padding(24.dp), verticalArrangement = Arrangement.spacedBy(20.dp)) {
            ...
            Text("原创双语例句 · ${state.editorialSize} 个词条")
            QuietText("考研义项频率只采用具有逐次出处、考试范围和复核记录的数据。缺数据标注「未统计」，保留词典原顺序；编辑优先级不参与频率排序。")
            QuietText("2024 参考词表含 5,530 条记录，规范化后 5,528 词均已收录。2027 英语（一）完整大纲尚未核验；参考词表不替代当年官方大纲。当前已核验义项频率覆盖 ${state.frequencySize} 词。")
            DataLicenses()
