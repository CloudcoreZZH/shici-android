...
**安装下载：[v0.2.0 Release](https://github.com/CloudcoreZZH/shici-android/releases/tag/v0.2.0)**。使用原签名，可直接覆盖安装 0.1.0。

[升级与使用说明](docs/releases/0.2.0/升级与使用说明.md) · [验证报告](docs/releases/0.2.0/验证报告.json) · [设计与学习规则](docs/0.2.0-设计与学习规则.md)

47 项自动化测试已通过，发布检查无错误。验证覆盖调度、重复加入、遗忘重学、撤销、进度恢复、旧版数据迁移，以及 Android 16 的横屏、大字号和深色界面；目标手机的实际表现仍需真机验收。本机构建交付位于 `dist/`，APK 通过 GitHub Release 分发，开发工具、缓存和签名私钥不提交到仓库。
...
| 学习首页 | 主动回忆 | 揭晓与评分 |
|---|---|---|
| ![学习首页](docs/screenshots/v0.2.0/home.png) | ![主动回忆](docs/screenshots/v0.2.0/question.png) | ![揭晓与评分](docs/screenshots/v0.2.0/answer.png) |
