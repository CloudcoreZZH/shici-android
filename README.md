...
**安装下载：[v0.3.0 Release](https://github.com/CloudcoreZZH/shici-android/releases/tag/v0.3.0)**。使用原签名，可直接覆盖安装 0.1.0 / 0.2.0。

[升级与使用说明](docs/releases/0.3.0/升级与使用说明.md) · [验证报告](docs/releases/0.3.0/验证报告.json) · [按天复习与动画](docs/0.3.0-按天复习与动画.md) · [免费词典质检](docs/免费词典质检与接入.md)

63 项自动化测试通过，发布检查无错误。覆盖按天调度、重复加入、遗忘等待、撤销、进度恢复、旧版迁移、真实词典检索和来源展示，以及 Android 16 的 120 Hz 请求、横屏、大字号和深色界面；目标手机的实际表现仍需真机验收。本机构建交付位于 `dist/`，APK 通过 GitHub Release 分发，开发工具、缓存和签名私钥不提交到仓库。

**尚未完成：2027 英语（一）完整大纲核验、全量义项编辑排序及全部中文释义的专业审校。** 已选择免费可信资料，未用旧年份词表或混合考试单词频数替代这些要求。
...
| 复习日历 | 主动回忆 | 揭晓与评分 |
|---|---|---|
| ![复习日历](docs/screenshots/v0.3.0/calendar.png) | ![主动回忆](docs/screenshots/v0.3.0/question.png) | ![揭晓与评分](docs/screenshots/v0.3.0/answer.png) |
...
- 浅色、深色和跟随系统；边到边布局、跟手预测性返回、原生 Compose 弹层与揭晓/换词动画。Android 16 使用 120 Hz 绘制请求，静止页不持续重绘，实际帧率由系统及设备决定。
- 使用手机已有的离线英语 TTS 语音。

**经用户确认，以明确标注的编辑释义优先级替代没有可靠数据的真题义项频率。** 当前覆盖 120 个词条、120 条原创双语说明例句，标注“编辑整理 · 非频率统计”和“原创用法例句 · 非真题”。完整 ECDICT 释义仍可查看；未覆盖词条保留原顺序。编辑版并非专业词典机构审校资料，覆盖范围也不是整个 76.9 万词库。
